---
description: Zoo Code 3.35 introduces resilient subtasks that persist across reloads and massive native tool calling expansion across 15+ providers.
keywords:
  - zoo code 3.35
  - subtasks
  - native tool calling
...
# Zoo Code 3.35 Release Notes (Combined)
...
<img src="/img/v3.35.0/v3.35.0.png" alt="Zoo Code v3.35 Release" width="600" />
...
* **Debug Buttons**: View API and UI history with new debug buttons (requires `zoo-cline.debug: true`) ([#9684](https://github.com/RooCodeInc/Roo-Code/pull/9684))
...
* **Malformed Tool Call Fix**: Fixed a regression where malformed native tool calls would cause Zoo Code to hang indefinitely. Tool calls now proceed to validation which catches and reports the missing parameters properly ([#9758](https://github.com/RooCodeInc/Roo-Code/pull/9758))
...
* **Stealth Model Privacy**: Models tagged with "stealth" in the Zoo API now receive vendor confidentiality instructions in their system prompt, enabling white-label or anonymous model experiences ([#9742](https://github.com/RooCodeInc/Roo-Code/pull/9742))
...
* **Zoo Provider Native Tools**: Models with the `default-native-tools` tag automatically use native tool calling by default ([#9735](https://github.com/RooCodeInc/Roo-Code/pull/9735))
...
* **App Version Tracking**: The Zoo provider now sends app version information with API requests for improved request tracking ([#9730](https://github.com/RooCodeInc/Roo-Code/pull/9730))
...
* **Zoo Provider Tool Compatibility**: Improved tool conversion for OpenAI-compatible API endpoints, ensuring tools work correctly with OpenAI-style request formats ([#9769](https://github.com/RooCodeInc/Roo-Code/pull/9769))
