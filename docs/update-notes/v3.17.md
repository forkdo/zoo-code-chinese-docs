---
description: Zoo Code 3.17 combines all v3.17.x updates including Intelligent Context Condensation, Boomerang improvements, and various bug fixes.
keywords:
  - zoo code
  - v3.17
  - release notes
...
# Zoo Code 3.17 Release Notes (2025-05-14)
...
Zoo Code now offers enhanced guidance for selecting the most appropriate mode for your tasks, primarily through the new "When to Use" field in mode definitions. This field allows mode creators to provide specific instructions on the ideal scenarios for using a particular mode. Previously, or if this field is not defined for a mode, Zoo would rely on the first sentence of the mode's role definition for this guidance.

*   **"When to Use" Field:** Custom modes can now include a "When to Use" description. This text is utilized by Zoo, especially the [Orchestrator (Boomerang) mode](/features/boomerang-tasks), to make more informed decisions when orchestrating tasks (e.g., via the [`new_task`](/advanced-usage/available-tools/new-task) tool) or when automatically switching modes (e.g., via the [`switch_mode`](/advanced-usage/available-tools/switch-mode) tool).
*   **Improved Orchestration:** By leveraging the "When to Use" field, Zoo can better understand the purpose of each mode, leading to more effective task delegation and mode selection.
*   **Fallback to Role Definition:** If the "When to Use" field is not populated for a mode, Zoo will use the first sentence of the mode's role definition as a default summary to guide its decisions.
...
We've made several under-the-hood refinements to improve how Zoo understands and responds to your requests:
...
*   **Automatic Summarization:** When a conversation approaches its context window limit (specifically, when the context window is almost full), Zoo Code now automatically uses a Large Language Model (LLM) to summarize the existing conversation history.
...
We've made a couple of nice tweaks to make your Zoo Code experience even better:

*   **Keep Typing, Even When Zoo's Thinking:** You can now type your next message in the chat even while Zoo is busy processing your current request. No more waiting for the input field to unlock – just keep your thoughts flowing!
*   **Stay Focused When Viewing Changes:** We've improved how Zoo Code handles your cursor focus when showing you code differences. This means fewer interruptions to your workflow when Zoo presents changes for review.

These improvements aim to make your interactions with Zoo Code feel more fluid and less disruptive.
...
*   Reverted the switch to the new Zoo message parser due to a tool parsing bug.
...
*   **Improved Developer Experience for Integrations**: We've added better support for developers building tools that interact with Zoo Code.
