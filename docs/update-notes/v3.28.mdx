---
description: Combined release notes for Zoo Code v3.28, featuring the Supernova model, Task Sync for mobile task management, Roo Code Cloud integration, improved chat editing, and various bug fixes.
keywords:
  - zoo code 3.28
  - supernova model
  - task sync
...
# Zoo Code 3.28 Combined Release Notes
...
Select the **FREE** code-supernova model from the Zoo provider to start using this powerful new capability at **no cost**.

> **Documentation**: See [Model Providers](/providers) for model selection details.
...
> **Important**: Zoo Code remains **completely free and open source**. Task Sync is an optional supplementary service that connects your local VS Code to the cloud - all processing still happens in your VS Code instance.
...
> **Documentation**: Task Sync was introduced in this release.
...
* **Social Media Sharing**: Improved OpenGraph images for better preview cards when sharing Zoo Code links on Slack, Twitter, and Facebook ([#8285](https://github.com/RooCodeInc/Roo-Code/pull/8285))
...
* **Welcome screen Zoo provider experiment**: Surfaces the Zoo provider on the welcome screen (localized, with click telemetry) to speed up setup. ([#8317](https://github.com/RooCodeInc/Roo-Code/pull/8317))
...
* **Fixed "No tool used" Errors**: Resolved the situation where LLMs would sometimes not make tool calls in their response which improves Zoo Code's overall flow. ([#8292](https://github.com/RooCodeInc/Roo-Code/pull/8292))
* **Context Condensing**: Fixed an issue where the initial task request was being lost during context condensing, causing Zoo Code to try to re-answer the original task ask when resuming after condensing ([#8298](https://github.com/RooCodeInc/Roo-Code/pull/8298))
* **Zoo provider stays signed in**: Zoo provider tokens refresh automatically and the local evals app binds to port 3446 for predictable scripts (via [#8224](https://github.com/RooCodeInc/Roo-Code/pull/8224))
...
* **Ollama respects Modelfile num_ctx**: Zoo Code now defers to your Modelfile’s context window to avoid GPU OOMs while still allowing explicit overrides when needed (via [#7798](https://github.com/RooCodeInc/Roo-Code/pull/7798); reported in [#7797](https://github.com/RooCodeInc/Roo-Code/issues/7797))
...
* **New Free Models**: Added two more free models to the Zoo provider - xai/grok-4-fast and deepseek/deepseek-chat-v3.1 ([#8304](https://github.com/RooCodeInc/Roo-Code/pull/8304))
