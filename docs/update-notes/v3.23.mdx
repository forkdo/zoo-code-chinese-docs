---
description: Zoo Code 3.23 graduates codebase indexing to stable, introduces task todo lists, command denylist, and Windows support for Claude Code.
keywords:
  - Zoo Code 3.23
  - release notes
  - codebase indexing
...
Codebase indexing has graduated from an experimental feature and is now a core part of Zoo Code, available directly from your chat input. Once configured, the indexer runs automatically in the background, ensuring Zoo always has an up-to-date semantic understanding of your project. To get started FREE, see the [Codebase Indexing quick start guide](/features/codebase-indexing#quick-start-guide).
...
This release introduces a new todo list feature to help you keep track of complex tasks. Zoo Code will now display a checklist of steps for your task, ensuring that no step is missed. You can view and manage the todo list directly in the chat interface.
...
- **Interface Organization**: Cleaner Zoo Code view header with reduced visual clutter - less frequently used actions moved to VS Code's overflow menu (thanks dlab-anton!) ([#3060](https://github.com/RooCodeInc/Roo-Code/pull/3060))
...
- **YouTube Footer**: Quick access to Zoo Code's YouTube channel from the website (thanks thill2323!)
- **PR Templates**: Issue-fixer mode now uses the official Zoo Code PR template (thanks MuriloFP!)
...
- **Error Handling**: Added error boundaries to prevent the entire Zoo Code interface from crashing when errors occur. Instead of seeing a blank screen, users now see helpful error messages with debugging information, allowing them to continue using other parts of the extension (thanks KJ7LNW, elianiva!)
...
- **Chat Focus**: Prevents the chat from losing focus during automated workflows, stopping user input from being accidentally redirected to files being edited by Zoo (thanks ColbySerpa!) ([#5349](https://github.com/RooCodeInc/Roo-Code/pull/5349))
...
- **Directory Tracking**: Fixed command execution directory tracking - Zoo Code now properly maintains awareness of the current working directory when executing commands, preventing issues where subsequent commands would run in the wrong location after using `cd` commands (thanks chris-garrett!) ([#5667](https://github.com/RooCodeInc/Roo-Code/pull/5667))
...
- **Process Termination**: Zoo Code now more reliably terminates unresponsive processes, preventing hanging commands from blocking your workflow
