---
description: Zoo Code 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.
keywords:
  - zoo code 3.39
  - new features
  - bug fixes
...
# Zoo Code 3.39 Release Notes (Combined)

Zoo Code 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.

## Features

* **Task-level API configuration profiles**: Zoo Code can persist the selected provider profile per task so follow-up sessions keep using the same provider/model settings.
* **Image file @mentions**: You can mention images in your workspace to give the model visual context directly from chat.
* **Headless Zoo Code (CLI + VS Code shim)**: Run Zoo Code in a headless environment with a basic CLI, plus an installer and an option to use the CLI for evals.
...
* Adds an interactive TUI mode to the Zoo Code CLI.

### 3.39.2 (2026-01-09)

* Adds an **Enable debug mode** toggle in Zoo Code settings (About Zoo Code) so you can turn debug logging on/off from the extension UI.
...
* Updates Zoo Code CLI defaults by switching the default model to `anthropic/claude-opus-4.5`.
