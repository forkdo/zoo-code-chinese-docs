---
description: Combined notes for Zoo Code 3.34, including Browser Use 2.0, the Baseten provider, OpenAI-compatible improvements, Zoo Code Cloud eval jobs, image generation UI tweaks, todo list cleanup, and cloud sync fixes.
keywords:
  - zoo code 3.34
  - browser use 2.0
  - baseten provider
...
# Zoo Code 3.34 Release Notes (2025-11-21)

Zoo Code 3.34 combines Browser Use 2.0, the Baseten provider, OpenAI-compatible improvements, and refined onboarding and native tool descriptions with patches that add Zoo Code Cloud eval support, clearer image generation prompts, todo list cleanup, cloud sync fixes, Claude Opus 4.5 across Zoo Code Cloud, OpenRouter, Anthropic, and Vertex, AWS Bedrock embeddings for code indexing, improved native tool workflows for Mistral and OpenAI-compatible providers, smoother Anthropic tool streaming on OpenRouter, better Bedrock global inference selection, plus new provider reliability and model updates, free Black Forest Labs image generation on Zoo Code Cloud, native MCP tool reliability fixes, native tool calling for Anthropic, Z.AI, and Moonshot providers, improved cloud sign-in experience, a race condition fix for the new_task tool on native protocol, and multiple bug fixes.

<img src="/img/v3.34.0/v3.34.0.png" alt="Zoo Code v3.34 Release" width="600" />

## Browser Use 2.0

Browser Use now supports a more capable "2.0" experience ([#8941](https://github.com/RooCodeInc/Roo-Code/pull/8941)):

- **Richer browser interaction**: Enables more advanced navigation and interaction patterns so Zoo can better follow multi-step web workflows.
...
- **Pinned Zoo provider**: Pinned the Zoo provider to the top of the provider list so it is easier to discover and select ([#9485](https://github.com/RooCodeInc/Roo-Code/pull/9485)).
- **Clearer native tool descriptions**: Enhanced built-in tool descriptions with examples and clarifications so Zoo can choose the right tools and use them more accurately ([#9486](https://github.com/RooCodeInc/Roo-Code/pull/9486)).
...
- **Eval jobs on Zoo Code Cloud**: You can now run evaluation jobs directly on Zoo Code Cloud models, reusing the same managed models and job tokens as regular cloud runs ([#9492](https://github.com/RooCodeInc/Roo-Code/pull/9492), [#9522](https://github.com/RooCodeInc/Roo-Code/pull/9522)).
...
- **Native tools as default for specific Zoo Code Cloud models**: Sets the providers’ native tool protocol as the default for `minimax/minimax-m2` and `anthropic/claude-haiku-4.5` on Zoo Code Cloud to reduce configuration overhead and improve tool-calling reliability ([#9586](https://github.com/RooCodeInc/Roo-Code/pull/9586)).
...
- **Zoo Code Cloud embeddings revert and reliability**: Removes Zoo Code Cloud as an embeddings provider to prevent `codebase_search` from appearing when it is not configured and to avoid indexing getting stuck in a standby state ([#9602](https://github.com/RooCodeInc/Roo-Code/pull/9602)).
...
- **Zoo Code Cloud image generation provider**: Zoo Code Cloud is now available as an image generation provider, so you can generate images directly through Zoo Code Cloud instead of relying only on third-party image APIs ([#9528](https://github.com/RooCodeInc/Roo-Code/pull/9528))).
...
- **Black Forest Labs image generation on Zoo Code Cloud**: Use the free `bfl/flux-2-pro:free` model on Zoo Code Cloud for high-quality image generation without unexpected charges, powered by the `images_api` image generation method for compatible models ([#9587](https://github.com/RooCodeInc/Roo-Code/pull/9587)).
...
- **AWS Bedrock embeddings for code indexing**: Adds support for using AWS Bedrock embeddings in code indexing so teams that standardize on Bedrock can reuse their existing infrastructure when indexing repos for Zoo-based navigation and search ([#9475](https://github.com/RooCodeInc/Roo-Code/pull/9475)).
