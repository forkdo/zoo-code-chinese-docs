---
description: Learn how to create reusable skills that package task-specific instructions, making Zoo Code more effective at specialized workflows.
keywords:
  - skills
...
Skills package task-specific instructions that Zoo loads on-demand when your request matches the skill's purpose. Unlike custom instructions that apply to everything, skills activate only when needed—making Zoo more effective at specialized tasks without cluttering the base prompt.
...
**Skills solve this**: Create a skill for PDF processing, and Zoo only loads those instructions when you actually ask to work with PDFs. This keeps the system prompt focused and gives Zoo deep expertise in specific domains without affecting unrelated tasks.
...
**Level 1: Discovery** - Zoo reads each `SKILL.md` file and parses its frontmatter to extract `name` and `description`. Only this metadata is stored for matching—the full content isn't held in memory until needed.

**Level 2: Instructions** - When your request matches a skill's description, Zoo uses [`read_file`](/advanced-usage/available-tools/read-file) to load the full SKILL.md instructions into context.

**Level 3: Resources** - The prompt tells Zoo it may access bundled files (scripts, templates, references) alongside the skill. There's no separate resource manifest—Zoo discovers these files on-demand when the instructions reference them.

This architecture means skills remain dormant until activated—they don't bloat your base prompt. You can install many skills, and Zoo loads only what's relevant for each task.
...
```bash
# Linux/macOS — Zoo-specific (higher priority)
...
- The description tells Zoo when to use this skill—be specific

#### 4. Test the skill

Ask Zoo something matching the description:
```
"Can you help me extract tables from this PDF file?"
```

Zoo should recognize the request matches your skill description, load the SKILL.md file, and follow its instructions.
...
```
~/.roo/skills/                    # Global Zoo-specific skills (high priority)
├── pdf-processing/
│   ├── SKILL.md                 # Required
...
.roo/skills/                      # Project Zoo-specific skills (override global)
└── custom-pdf-workflow/
    └── SKILL.md
...
```
~/.roo/skills-code/              # Only in Code mode (Zoo-specific, global)
└── refactoring-patterns/
    └── SKILL.md

.roo/skills-architect/           # Only in Architect mode (Zoo-specific, project)
...
Zoo automatically discovers skills:
...
**Symptom**: Zoo doesn't use your skill even when you request something matching the description.
...
4. **Description too vague**: Make descriptions specific so Zoo can match them to requests
   ```markdown
   # ✗ Vague
...
**Symptom**: Zoo reads the skill but doesn't follow instructions.
...
**Symptom**: Unclear which skill Zoo will use when multiple might match.
...
Zoo Code skills follow the [Agent Skills](https://agentskills.io/) format for skill packaging and metadata. Skills are **instruction packages with optional bundled files**—they don't register new executable tools.
...
#### Zoo-specific enhancements

Zoo Code adds mode-specific targeting and cross-agent compatibility beyond the base format:

- **Zoo-specific locations**: `.roo/skills/`, `~/.roo/skills/` (higher priority)
