---
description: Learn about the experimental Background Editing setting that allows uninterrupted coding while Zoo Code makes file edits in the background.
keywords:
...
Work without interruption while Zoo Code edits files in the background—no more losing focus from automatic diff views.
...
The "Background Editing" setting is an experimental feature that disables automatic diff view displays when Zoo Code edits files. Instead of switching your editor focus to show diffs, Zoo works silently in the background, allowing you to continue coding without interruption. This feature affects all file editing operations including write, apply diff, search/replace, insert content, and multi-file apply diff tools.

### Key Benefits

- **Uninterrupted Focus**: Stay in your current file while Zoo makes changes
...
- **Limited Environment Context**: Zoo won't see recently edited files as open tabs in its environment details since they're not visually opened
...
1. Open Zoo Code settings (gear icon in the top right)
2. Navigate to the "Experimental" tab
3. Find "Background editing" in the list
4. Toggle the setting to enable it

<img src="/img/background-editing/background-editing.png" alt="Background editing setting in Zoo Code experimental features" width="400" />
...
Without this feature, when Zoo edits a file:
...
- Zoo's chat shows what files were edited
...
- **Large Refactoring Operations**: When Zoo needs to update many files
...
4. **Monitor Chat**: Pay attention to Zoo's messages about file modifications
...
**Q: Can I still see what files Zoo edited?**
A: Yes, Zoo's chat messages list all modified files, and changes appear in source control.
...
**Q: Does this affect Zoo's ability to edit files?**
A: No, Zoo can still make all the same edits; only the display behavior changes. All file editing tools (write, apply diff, search/replace, insert content, and multi-file apply diff) respect this setting.
