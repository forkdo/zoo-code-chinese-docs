...
description: Manage your Zoo Code settings by exporting, importing, or resetting them to defaults.
keywords:
  - settings management
  - import settings
  - export settings
...
Zoo Code allows you to manage your configuration settings effectively through export, import, and reset options. These features are useful for backing up your setup, sharing configurations with others, or restoring default settings if needed.

You can find these options at the bottom of the Zoo Code settings page, accessible via the gear icon (<i class="codicon codicon-gear"></i>) in the Zoo Code chat view.

<img src="/img/settings-management/settings-management.png" alt="Export, Import, and Reset buttons in Zoo Code settings" width="400" />
...
Clicking the **Export** button saves your current Zoo Code settings to a JSON file.
...
*   **Process:**
    ...
    3.  Zoo Code reads the file, validates its contents against the expected schema, and applies the settings.
...
*   **Validation:** Import validates the file, but it can still succeed with warnings.
    ...
    - If **some** API profiles reference a provider that no longer exists (or is otherwise invalid), Zoo imports the rest and reports warnings.
...
Automatically import your Zoo Code settings from a file every time you start VS Code. This is a practical way to sync your configuration across multiple machines or standardize settings for your entire team.
...
- **Silent & Safe**: If the file isn't found or contains errors, Zoo Code starts up normally without blocking your workflow.
...
**With this feature**: Configure the path once, and Zoo Code handles the rest on every launch.

### How it Works

When VS Code starts, Zoo Code checks for a specific setting: `roo-cline.autoImportSettingsPath`. If this setting contains a path to a valid Zoo Code configuration file (`.json`), Zoo Code will load it automatically.
...
- Zoo Code will show a warning notification with the error details. The extension will continue to load normally with your previously saved settings.

**"Where does Zoo Code look for relative paths?"**
...
Clicking the **Reset** button completely clears all Zoo Code configuration data and returns the extension to its default state. This is a destructive action intended for troubleshooting or starting fresh.
...
*   **What is Reset:**
    ...
    *   **Secret Storage:** All API keys and other secrets managed by Zoo Code are cleared.
    *   **Task History:** The current task stack is cleared.

*   **Result:** Zoo Code returns to its initial state, as if freshly installed, with default settings and no user configurations.

Use this option only if you are certain you want to remove all Zoo Code data or if instructed during troubleshooting. Consider exporting your settings first if you might want to restore them later.
...
Opens a dialog to set a custom storage directory for Zoo Code data. By default, Zoo Code stores task history, settings, and other data in the standard VS Code extension storage location. This command allows you to choose an alternative location.

**Use cases:**
- **Team Collaboration**: Store Zoo Code data in a shared network folder so team members can access the same task history and settings
...
Imports Zoo Code settings from a JSON file via the Command Palette. This is an alternative to using the Import button in the settings UI.
...
Zoo Code provides VS Code settings that can be configured through your VS Code `settings.json` file. These settings offer fine-grained control over command execution, task management, API behavior, storage, indexing, and debugging.
...
- **Description**: Commands that can be auto-executed without approval. When Zoo Code requests to execute a command that matches an entry in this list, it will execute automatically without prompting for approval. This is useful for safe, read-only commands.

#### `roo-cline.deniedCommands`
- **Type**: Array of strings
- **Default**: `[]`
- **Description**: Commands that are always blocked from execution. Zoo Code will refuse to execute any command that matches an entry in this list, providing a safety mechanism to prevent potentially dangerous operations.
...
- **Type**: Boolean
- **Default**: `false`
- **Description**: Prevents task completion when there are uncompleted todo items. When enabled, Zoo Code will not allow you to mark a task as complete if the todo list still has pending items, ensuring all planned work is finished.
...
- **Default**: `600`
- **Range**: 0-3600
- **Description**: Timeout in seconds for API requests. Determines how long Zoo Code will wait for a response from AI provider APIs before timing out. A value of 0 means no timeout.
...
- **Type**: String
- **Default**: `""` (empty)
- **Description**: Custom file path for Zoo Code's storage directory. By default, Zoo Code stores its data in the standard extension storage location. Use this setting to specify an alternative directory for storing task history, settings, and other data.

#### `roo-cline.autoImportSettingsPath`
- **Type**: String
- **Default**: `""` (empty)
- **Description**: File path for automatic settings import on startup. When configured, Zoo Code will automatically import settings from the specified JSON file every time VS Code starts. See the [Automatic Configuration Import](#automatic-configuration-import) section above for detailed usage instructions.
...
- **Default**: `10000`
- **Range**: 5000-500000
- **Description**: Maximum number of files indexed for file search. Controls the upper limit of files that Zoo Code will index for semantic search functionality. Higher values increase search coverage but may impact performance.
...
- **Type**: Boolean
- **Default**: `true`
- **Description**: Controls whether Zoo Code actions appear in the editor context menu and lightbulb. When enabled, you can right-click in the editor or use the lightbulb menu to quickly send code selections to Zoo Code with contextual prompts.
...
- **Type**: Boolean
- **Default**: `true`
- **Description**: Enable loading of AGENTS.md files for agent-specific instructions. When enabled, Zoo Code will look for and load `AGENTS.md` files in your project directories to provide context-specific guidance to the AI. Disable this if you want to prevent automatic loading of these instruction files.
...
- **Type**: Boolean
- **Default**: `false`
- **Description**: Enable debug mode for additional logging. When enabled, Zoo Code will output detailed debug information to the console, useful for troubleshooting issues or understanding internal behavior.
