...
As your conversation with Zoo Code grows, it might approach the context window limit of the underlying AI model. When this happens, older messages would typically be removed to make space. Intelligent Context Condensing aims to prevent this abrupt loss by:
...
1.  Open Zoo Code settings (<Codicon name="gear" /> icon in the top right corner of the Zoo Code panel).
...
3.  Configure the available options:
    ...
    - **Custom Context Condensing Prompt**: Customize the prompt Zoo uses when condensing (found in "Context Management" settings)
...
Zoo Code provides several ways to control and understand the Intelligent Context Condensing feature:

#### Controlling context condensing
*   **Automatic Threshold:** The threshold slider in "Context" settings allows you to define a percentage (e.g., 80%) of context window usage. Zoo Code will attempt to condense the context automatically when the conversation reaches this level of capacity.
...
*   **Context Condensing Metrics:** When context condensing occurs, Zoo Code displays:
...
*Why this view matters: it’s your audit trail for what changed, how much it cost, and what Zoo will carry forward.*
...
1. Go to Zoo Code settings (<Codicon name="gear" /> icon)
2. Open **Context Management**
3. Find the **Custom Context Condensing Prompt** editor
4. Enter your custom prompt that instructs Zoo on what must be preserved
...
When Zoo Code encounters context window limit errors, it now automatically recovers to keep your work flowing:

#### How error recovery works

1. **Error Detection**: Zoo Code detects context window errors from multiple providers (OpenAI, Anthropic, and others)
2. **Automatic Truncation**: The system automatically reduces the context by 25%
3. **Retry Mechanism**: After truncation, Zoo Code retries your request (up to the built-in retry limit)
4. **Continuation**: Zoo retries without manual intervention
...
#### Token counting
Zoo Code uses a sophisticated token counting system that:
...
- If you had a legacy `customCondensingPrompt`, Zoo migrates it into this location automatically.
- **Reset** clears the override so Zoo falls back to the built-in default condense prompt.
