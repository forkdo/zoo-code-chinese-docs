---
description: Learn how to create and use custom slash commands in Zoo Code to automate workflows and extend functionality with markdown-based definitions.
keywords:
  - slash commands
  - custom commands
  - Zoo Code commands
...
Create custom slash commands to automate repetitive tasks and extend Zoo Code's functionality with simple markdown files.
...
Custom commands extend Zoo Code's functionality by adding markdown files to specific directories:
...
- **`mode`**: (Optional) Mode slug to switch to before running the command (e.g., `code`, `architect`). Zoo switches to this mode first, then executes the command content in that mode's context
...
1. Click the gear icon in Zoo Code and open Settings
...
When you run this command, Zoo switches to Architect mode first, then processes the command content with Architect's role definition and tool restrictions.
...
Zoo Code includes powerful built-in commands that provide specialized functionality:
...
- [Using Modes](/basic-usage/using-modes) - Learn about Zoo Code's different operational modes
- [Custom Instructions](/features/custom-instructions) - Set persistent instructions for Zoo Code
