...
description: Access over 100 LLMs through LiteLLM's unified OpenAI-compatible API in Zoo Code. Simplify multi-model management and reduce costs.
keywords:
  - litellm
  - zoo code
  - api provider
...
*   **Simplified Configuration:** Manage credentials and model configurations in one place (your LiteLLM server) and let Zoo Code connect to it.
...
To use LiteLLM with Zoo Code, you first need to set up and run a LiteLLM server.
...
Once your LiteLLM server is running, you have two options for configuring it in Zoo Code:
...
4.  **Enter API Key (Optional):**
    ...
    *   If your LiteLLM server doesn't require an API key, Zoo Code will use a default dummy key (`"dummy-key"`), which should work fine.
5.  **Select Model:**
    *   Zoo Code will attempt to fetch the list of available models from your LiteLLM server by querying the `${baseUrl}/v1/model/info` endpoint.
...
<img src="/img/litellm/litellm.png" alt="Zoo Code LiteLLM Provider Settings" width="600" />

---

## How Zoo Code Fetches and Interprets Model Information

When you configure the LiteLLM provider, Zoo Code interacts with your LiteLLM server to get details about the available models:

*   **Model Discovery:** Zoo Code makes a GET request to `${baseUrl}/v1/model/info` on your LiteLLM server. If an API key is provided in Zoo Code's settings, it's included in the `Authorization: Bearer ${apiKey}` header.
*   **Model Properties:** For each model reported by your LiteLLM server, Zoo Code extracts and interprets the following:
...
*   **LiteLLM Server is Key:** The primary configuration for models, API keys for downstream providers (like OpenAI, Anthropic), and other advanced features are managed on your LiteLLM server. Zoo Code acts as a client to this server.
...
*   **Model Availability:** The models available in Zoo Code's "Model" dropdown depend entirely on what your LiteLLM server exposes through its `/v1/model/info` endpoint.
*   **Network Accessibility:** Ensure your LiteLLM server is running and accessible from the machine where VS Code and Zoo Code are running (e.g., check firewall rules if not on `localhost`).
*   **Troubleshooting:** If models aren't appearing or requests fail:
    ...
    *   Ensure the Base URL in Zoo Code settings matches your LiteLLM server's address.
    *   Confirm any API key required by your LiteLLM server is correctly entered in Zoo Code.
*   **Computer Use Models:** The `supportsComputerUse` flag in Zoo Code is primarily relevant for certain Anthropic models known to perform well with tool-use and function-calling tasks. If you are routing other models through LiteLLM, this flag might not be automatically set unless the underlying model ID matches the specific Anthropic ones Zoo Code recognizes.

By leveraging LiteLLM, you can significantly expand the range of models accessible to Zoo Code while centralizing their management.
